...
def execute_graph(graph: Graph,
    ...
    for i, (dists, rep) in enumerate(zip(graph[1:], seq)):
        ...
        def calc_formula(ancestor: tuple) -> str:
            if ancestor[0] == 'zz':
                # return f"{formula[ancestor[1]]} * R_zz({i})"
                return f"{formula[ancestor[1]]} * {z_to_z.mean().item():.3}"
            elif ancestor[0] == '++':
                # return f"{formula[ancestor[1]]} * R_++({i})"
                return f"{formula[ancestor[1]]} * {p_to_p.mean().item():.3}"
            elif ancestor[0] == 'z+':
                # return f"{formula[ancestor[1]]} * R_z+({i})"
                return f"{formula[ancestor[1]]} * {z_to_p.mean().item():.3}"
            elif ancestor[0] == '+z':
                # return f"{formula[ancestor[1]]} * R_+z({i})"
                return f"{formula[ancestor[1]]} * {p_to_z.mean().item():.3}"
            elif ancestor[0] == '-z':
                # return f"({formula[ancestor[1]]})^* * R_-z({i})"
                return f"({formula[ancestor[1]]})^* * {m_to_z.mean().item():.3}"
            elif ancestor[0] == '-+':
                # return f"({formula[ancestor[1]]})^* * R_-+({i})"
                return f"({formula[ancestor[1]]})^* * {m_to_p.mean().item():.3}"
            else:
                raise ValueError(f"Unknown transform {ancestor[0]}")
        ...
        for dist in dists:
            ...
            if i < 5:
                formula[dist] = " + ".join(calc_formula(ancestor) for ancestor in ancestors)
            ...
            if dist.dist_type == '+':
                ...
                if i < 5:
                    # formula[dist] = f"{formula[dist]} * exp(-T({i})/T2 - b({i})*D)"
                    formula[dist] = f"{formula[dist]} * {(r2 * diffusion[-1, :]).mean().item():.3f}"
                ...
                if i < 5:
                    # formula[dist] = f"{formula[dist]} * exp(-T({i})/T1 - b({i})*D)"
                    formula[dist] = f"{formula[dist]} * {(r1 * diffusion).mean().item():.3f}"
                dist.mag = dist.mag * r1 * diffusion
            if dist.dist_type == 'z0':
                if i < 5:
                    # formula[dist] = f"{formula[dist]} + (1 - exp(-T({i})/T1))"
                    formula[dist] = f"{formula[dist]} + {(1 - r1).mean().item():.3f}"
