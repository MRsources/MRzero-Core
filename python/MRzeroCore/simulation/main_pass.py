...
def execute_graph(graph: Graph,
    ...
    """
    # Calculate additional phase generated by translation motion
    time = torch.cat([torch.zeros(1)] + [rep.event_time for rep in seq]).cumsum(0)
    translation = torch.tensor([data.motion(t) for t in time], dtype=torch.float)
    avg_pos = translation[1:, :] - translation[:-1, :]

    gradm = torch.cat([rep.gradm for rep in seq])
    motion_phase = (gradm * avg_pos).sum(1).cumsum(0)
    motion_phase = torch.split(motion_phase, [rep.event_count for rep in seq])
    ...
    mag_z = []
    for i, (dists, rep, mp) in enumerate(zip(graph[1:], seq, motion_phase)):
        if print_progress:
